...
public class MainActivity2 extends Activity {
	...
	long lastTime;//记录上次 保存到历史记录中的数据
    ...
    public static final int handler_LinkSuccess = 102;
		 ...
		 private final int handler_switch_web = 108;//云端在线
	...
	public Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			switch(msg.what) {
			case handler_toast:
				ToastNew.makeTextMid(MainActivity2.this,R.string.link_auto, 3).show();
				break;
				case handler_Link:
					ToastNew.makeTextMid(MainActivity2.this, R.string.linking, 3).show();
					break;
				case handler_LinkSuccess:
					ToastNew.makeTextMid(MainActivity2.this,R.string.link_success, 3).show();
					DeviceStatusBin.getInstance().setDeviceSwitch(true);
					isShowToast = false;
					mac = mSetupData.read(SetupData.LastBluetooth);
					mConnService.writeToAllSockets(new byte[]{ (byte) 0xaa, (byte) 0x51, (byte) 0x52, (byte) 0x53, (byte) 0x54, (byte) 0x00 });
					break;
				case handler_LinkFailure:
					if(isShowToast) {
						ToastNew.makeTextMid(MainActivity2.this, R.string.link_fail, 3).show();
						isShowToast = false;
					}
...
//						 System.out.println("接收成功" + temperValue+" " + (Myhex.buffer2String(buf)) +buf[0]);
...
//								System.out.println( " 添加成功 " +hsqlService.insert(hbin) + " " + hbin.getValue() + " " + hbin.getDate());
//							}
//						} else {
//							DeviceStatusBin.getInstance().setTemper(temperValue);
//						}
...
//					break;
			  case MESSAGE_READ:	            
				...
				case MESSAGE_WRITE:
					...
					System.out.println("写出" + Myhex.buffer2String(buff));
					break;
				...
				case handler_login_unuse:
					Toast.makeText(MainActivity2.this, R.string.login_unuse, 3).show();
					break;
			}
	...
	private void parseData(byte[] buf) {
		int value;
		int ele;//电量
		 System.out.println("接收成功" + " " + (Myhex.buffer2String(buf,buf.length)));
		switch(buf[0]) {
		case (byte) 0xAA://温度
			value = (MyByte.byteToInt(buf[1]) * 256 + MyByte.byteToInt(buf[2]));
			//if(value>3200 && value <4500) {//只有有效的温度 ， 才记录
					...
					new Thread(new Runnable(){
						...
						public void run() {
							...
							if(AppConstants.isLineMode) {
								System.out.println( " 添加在线");
								//本软件温度数据， 是扩大了100倍数， 服务器上的是正常的 小数 , 所以将温度  除以100.0d
								startSyncThread(hbin, ""+voltage, ""+MyDecimalHelp.getRoundFolat2(alert_temper/100.0f), ""+alarmStatus);
							} else {//离线模式
								hbin.setOnline(false);
								//lastTime = nowTimeMillis;
								System.out.println( " 添加离线成功 " + 	hsqlService.insertByMac(hbin)+" " + hbin.getValue() + " " + hbin.getDate());
							}
							
						}}).start();
					
				} else{
					System.out.println( " 间隔时间未到" + (nowTimeMillis - lastTime));
				}
//			} else {
//				DeviceStatusBin.getInstance().setTemper(value);
//			}
		...
		case (byte) 0xCC:
			...
			default:
				if(value<=344){
					DeviceStatusBin.getInstance().setDeviceSwitch(false);
					mConnService.writeToAllSockets(new byte[]{ (byte) 0x31, (byte) 0x32, (byte) 0x33, (byte) 0x34, (byte) 0x35, (byte) 0x36 });
			...
			break;
		case (byte) 0xbb:
			//Toast.makeText(MainActivity2.this, "校正发送成功", Toast.LENGTH_SHORT).show();
			break;
		case (byte) 0xdd:
			if(buf[1] == ((byte) 0xee)) {
		...
		case (byte) 0xe0:
			 value = MyByte.byteToInt(buf[1]); 
				 ...
				 //点亮
				 int valueEle = MyByte.byteToInt(buf[4]) * 256 + MyByte.byteToInt(buf[5]);;//电量
					switch(valueEle) {
					case 404:
						ele = 100;
						break;
...
//				}
				...
				mac = data.getStringExtra(DeviceListActivity2.EXTRA_DEVICE_ADDRESS);
		...
		case activity_OpenBlueTooth:
			if( resultCode == Activity.RESULT_OK) {
				...
				ToastNew.makeTextMid(MainActivity2.this, getString(R.string.noBluetooth1),
...
//					}
//					isShowLowAlert = true;
//					mConnService.writeToAllSockets(new byte[]{ (byte) 0x91, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) (alertValue/10 -350) });
//				}
//			});
//			confirm_btn.setOnClickListener(new OnClickListener() {
//				
...
//	}
	...
	private void showNotification(String content, boolean isrecly) {
		...
		if(notificationManager == null) {
			notificationManager = (NotificationManager) MainActivity2.this
		...
		int ringSound = RingHelp.getRingRourse(MainActivity2.this, mSetupData.readInt(AppString.Ring, 0));
...
//				| Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);// 关键的一步，设置启动模式
		        ...
		        public void onReceive(Context context, Intent intent) {
		            ...
		            try {
		            	 if(action.equals("android.bluetooth.device.action.PAIRING_REQUEST")) {
		                		...
		                		if(!AppConstants.isLineMode) {
		                			if(SocketHelper.isnetworkConnected(MainActivity2.this)) {
	...
	int count= 59;//%20，查询一次状态，%30 查询一次 广告 

	Runnable linkRunnable = new Runnable() {//这里是自动连接线程， 现在将自动连接 ， 和自动去服务器查询广告，合并，一起用， 20次自动连接，就查询一次服务器广告
		...
		public void run() {
			...
			while(true) {
				...
				if(isShowToast) {//表示，正在处于 手动连接中， 就退出自动连接
					continue;
				}
				if(mConnService!=null && mConnService.isLink()) {
					continue;
...
//	};
	...
	private void startQueryStatus() {
		...
		if(mConnService.isLink()) {//如果已经是连接上设备就不查询
			return;
		}
	...
	Runnable queryStatus = new Runnable() {
		...
		public void run() {
			...
			if(mNetworkManager.isNetworkSuccess(result)) { //获得服务器上的温度状态
...
//					}
	...
	private void startSyncThread( HistoryBin historyBin,final String voltage,final String alarmTemperature,final String alarmState) {
		if(isDestroy) return;
		if(!mConnService.isLink()) {//没有连接上设备
			return;
		}
//				// TODO Auto-generated method stub
				...
				} else if(mNetworkManager.isNetworkNoLogin(result)) {  //登入已经失效
					String password = mSetupData.read(AppString.password, "");
					if(password.equals("")) {//没有密码
